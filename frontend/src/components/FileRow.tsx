...
export default function FileRow({
  ...
  const displayTitle = `${file.trackNumber ? `${file.trackNumber}. ` : ""}${file.title ?? file.filename}`;
    ...
    <div className={`flex w-full items-center gap-3 rounded px-2 py-2 hover:bg-slate-800 ${isCurrent ? "bg-slate-800" : ""}`}>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          {/* title attr so a truncated long name shows in full on hover. */}
          <div className="truncate text-sm" title={displayTitle}>
            {displayTitle}
          </div>
